...
from data_for_tests import const, locators as loc, config
...
class TestPersonalAccount:
    ...
    def test_log_in_main_page(self, button, start_url, driver):
        ...
        button_from_params = (WebDriverWait(driver, 10).
                              until(expected_conditions.visibility_of_element_located(button)))
        button_from_params.click()
        (WebDriverWait(driver, 10).
         until(expected_conditions.visibility_of_element_located(loc.HEADER_LOGIN_PAGE)))
    ...
    def test_go_to_constructor_and_logo(self, button, driver, log_in):
    ...
    def test_go_to_pa_and_exist(self, driver, log_in):
        button_account = (WebDriverWait(driver, 10).
                          until(expected_conditions.visibility_of_element_located(loc.BUTTON_ACCOUNT)))
        button_account.click()
        logout_button = (WebDriverWait(driver, 10).
                         until(expected_conditions.visibility_of_element_located(loc.BUTTON_LOGOUT)))
        logout_button.click()
        (WebDriverWait(driver, 10).
         until(expected_conditions.visibility_of_element_located(loc.HEADER_LOGIN_PAGE)))
        assert driver.current_url == const.login_page
